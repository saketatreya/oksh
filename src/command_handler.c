...
#include "proclore.h"
#include "seek.h"
#include "activities.h"  // Include activities header
#include "config.h"  // Include config header for handling functions and aliases
#include "neonate.h"  // Include neonate command header
#include "io_redirection.h"  // Include for I/O redirection
#include "iman.h"
#include "fgbg.h"
#include "ping.h"
#include "log.h"
...
void handle_command(char* command, const char* home_directory, int run_in_background) {
    ...
    // Handle ping command (for sending signals)
    if (strcmp(args[0], "ping") == 0) {
        if (argc > 2) {
            pid_t pid = atoi(args[1]);
            int signal_num = atoi(args[2]);
            ping_command(pid, signal_num);
        } else {
    ...
    if (strcmp(args[0], "neonate") == 0) {
        if (argc > 2 && strcmp(args[1], "-n") == 0) {
            int interval = atoi(args[2]);  // Parse time argument
            neonate_command(interval);
        } else {
            printf("Invalid usage of neonate command. Use: neonate -n [time_arg]\n");
        }
        return;
    }
    ...
    if (strcmp(args[0], "hop") == 0) {
        ...
        if (strstr(original_command, ">") != NULL || strstr(original_command, "<") != NULL || strstr(original_command, "|") != NULL) {
            handle_pipes_and_redirection(original_command, home_directory);
            return;
        }
        ...
        if (pid == 0) {
            ...
            add_process_to_activities(original_command, pid);
            ...
            if (run_in_background) {
                ...
                // Update process state if it was stopped
                if (WIFSTOPPED(status)) {
                    update_process_state(pid, 0);  // Mark as stopped
                } else if (WIFEXITED(status)) {
                    update_process_state(pid, 0);  // Mark as stopped when exited
                }
            }
        } else {
            // Fork failed
...
void parse_and_execute(char* input, const char* home_directory) {
    ...
    while (command_part != NULL) {
        ...
        if (replace_alias(command_part)) {
        }

        // Check if it's a function next
        if (execute_function(command_part, home_directory)) {
        } else {
    ...
    while ((finished_pid = waitpid(-1, &status, WNOHANG)) > 0) {
        if (WIFEXITED(status)) {
            ...
            update_process_state(finished_pid, 0);  // Mark as stopped when exited
        } else if (WIFSTOPPED(status)) {
            printf("Background process %d was stopped.\n", finished_pid);
            update_process_state(finished_pid, 0);  // Mark as stopped
        } else {
